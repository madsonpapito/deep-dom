...
const stages = [
  {
    ...
    lessons: [
      { id: "0-1", title: "O Início da sua Alforria", icon: "🏠" },
      { id: "0-2", title: "Como navegar no portal", icon: "🧭" },
      { id: "0-3", title: "Segurança em Primeiro Lugar", icon: "🛡️" }
    ]
  },
  {
    title: "Etapa 1: Desafio Despensa Blindada",
    lessons: [
      { id: "1-1", title: "Dia 1: Auditoria da Farmácia", icon: "📋" },
      { id: "1-2", title: "Dia 2: O Despertar da Planta", icon: "🌡️" },
      { id: "1-3", title: "Dia 3: O Elixir do Estômago", icon: "🛡️" },
      { id: "1-4", title: "Dia 4: O Protocolo das Juntas", icon: "🦵" },
      { id: "1-5", title: "Dia 5: O Mapa da Independência", icon: "🗺️" }
    ]
  },
  {
    title: "Etapa 2: Sistema SAB (Ciência)",
    lessons: [
      { id: "2-1", title: "A Ciência do Calor", icon: "🧪" },
      { id: "2-2", title: "Rompendo a Parede Celular", icon: "🧬" },
      { id: "2-3", title: "Veículos Lipídicos", icon: "🚚" }
    ]
  }
];
              ...
              item.download ? (
                <a key={item.id} href={item.link} download className="group">
                  <div className="bg-white p-4 rounded-2xl shadow-md border border-gray-100 group-hover:border-forest/30 group-hover:shadow-xl transition-all flex items-center gap-4 h-full">
                    ...
                    <div className="flex-1">
                      ...
                      <h3 className="font-bold text-gray-900 leading-tight text-sm mt-1">{item.title}</h3>
                ...
                <Link key={item.id} href={item.link} className="group">
                  <div className="bg-white p-4 rounded-2xl shadow-md border border-gray-100 group-hover:border-forest/30 group-hover:shadow-xl transition-all flex items-center gap-4 h-full">
                    ...
                    <div className="flex-1">
                      <span className={`text-[10px] font-black py-0.5 px-2 rounded-full ${item.type === 'UPSELL' ? 'bg-gold/10 text-gold' : 'bg-forest/10 text-forest'} uppercase`}>
                        {item.type}
                      </span>
                      <h3 className="font-bold text-gray-900 leading-tight text-sm mt-1 group-hover:text-forest">{item.title}</h3>
                      <p className="text-[10px] text-gray-500 mt-1 line-clamp-1 leading-tight">{item.description}</p>
                    </div>
                  </div>
                </Link>
